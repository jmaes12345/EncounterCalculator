...
D&D 5e Encounter Calculator — a web app for game masters to calculate encounter difficulty based on PC party levels and enemy XP values. It has three modules:

- `app/backend/` — Java/Micronaut REST API (stateless calculation engine)
- `app/frontend/encounter-calculator/` — Angular 10 web app
- `app/PCThresholdsFunction/` — Google Cloud Functions (Java) alternative deployment
...
```bash
cd app/PCThresholdsFunction
mvn function:run -Drun.functionTarget=functions.PCThresholds
```

## Architecture

### Backend
Micronaut 2.x on Netty. No persistence — pure stateless calculation engine.
...
- **Models** (`model/`) — `PCParty`/`PC`, `EnemyParty`/`Enemy`, `Difficulty` enum
